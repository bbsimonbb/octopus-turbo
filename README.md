...
Most computer programs, and all user interfaces, are directed acyclic graphs. They can be modelled as object-oriented systems, or event-driven ones, or with a functional paradigm, but they _intrinsically are_ directed acyclic graphs, because this is the loosest possible structure that doesn't admit infinite loops. So let's see what happens when we make this structure explicit...

**Octopus state graph** constructs a graph out of your nodes. A node has a name, a value (`val`), `methods` and a recalculate function, called `reup()` because it's shorter and I'm a huge Wire fan. `reup()` _can reference any other node by name in its argument list._ This is all we need to build the graph. The `methods` property contains all the methods that will modify `val`. (This is your responsibility. Only a node's own `reup()` and `methods` should modify its `val`.) 
