...
**Octopus state graph** constructs a graph out of your nodes. A node has a name, a value (`val`), `methods` and a recalculate function, called `reup()` because it's shorter and I'm a huge Wire fan. `reup()` _can reference any other node by name in its argument list._ The `methods` property contains all the methods that will modify `val`. (This is your responsibility. Only a node's own `reup()` and `methods` should modify a its `val`.) 
