...
To integrate with a front-end framework, we just need the framework to observe the node. In Vue, this is easily accomplished by wrapping the node in `reactive()`. For react we need to grab mobx, and wrap the node with `makeAutoObservable()`, and our react components with `observer()`. Additionally, mobx likes you to tag state-modifying functions as actions, so that all actions can complete before the DOM is modified. If you pass `mobx.action()` to octopus, octopus will look after this for you. You should also wrap any callbacks you create inside `reup()` and node functions. All this is demonstrated in the React sample.
